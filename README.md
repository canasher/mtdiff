...
| `--allow-structure-truncate`（仅 sync） | off | 原地结构 ALTER **失败**时，回退到旧的"先 `TRUNCATE` 再重放 DDL"路径。默认关闭：原地 ALTER 失败只报错、**dst 数据原样保留**（InnoDB 的 ALTER 是原子的） |
...
- sync 默认**先对齐表集与表结构，再写数据**：
  ...
  - **结构修复默认原地 `ALTER`，不再先 `TRUNCATE`**（InnoDB 的 ALTER 是原子的）：DDL 之后重读 dst 元数据、重新比较、按修后的结构**重新规划**（修回了可用键的表回到行级同步；仍无键的才走全量重灌，且只在确认全量重灌前才 `TRUNCATE`）。原地 ALTER **失败**（如 dst 现有数据装不进 src 的列类型）时默认**只报错、dst 数据原样保留**（exit 2，零写入）；`--allow-structure-truncate` 显式回退到旧的"先 `TRUNCATE` 再重放 DDL"路径。
  - **生成列安全拒绝**：源表含生成列（`GENERATED ALWAYS AS`，VIRTUAL/STORED）时结构同步**不尝试复现**生成表达式（跨后端不可靠），该表报运行错（exit 2）——对齐 schema 或 `--no-sync-schema`。数据路径上生成列**参与比较、永不写入**（`INSERT`/`UPDATE` 不含它，由 dst 自行推导）。两侧生成列定义一致时不算漂移、不产生 DDL。
...
- E2E：`make e2e`（docker 双 MySQL 8.0 实例；"不同时区"由种子在会话级 `SET time_zone`（+08:00 vs -04:00）模拟，而非服务端 `system_time_zone`，见 `e2e/docker-compose.yml` 头注释；229 项断言（141 退出码 + 88 输出内容），覆盖退出码 / JSON 报告 / 并行指纹确定性，含 sync 的 dry-run 零写入 / row-level（dst 多行只删不灌）/ 无键表全量 / `--where` 零匹配删除 / 范围外行删除（int / 复合 / VARCHAR / NULL 键、`--where` 残留、无 TRUNCATE 首轮收敛）/ 结构漂移自动对齐（DDL 展示、零写入、information_schema 内容断言、修复后重新规划、`--no-sync-schema` 回归）/ 键漂移（一侧有键一侧无键：diff 全表多重集回退、默认结构同步补回主键后回到行级）/ 整库同步（缺表建表、多余表 DROP、表状态 AUTO_INCREMENT 收敛、`--tables` 不删、`--exclude-tables` 豁免、缺表 + `--where`/`--no-sync-schema` 报错）/ 参数错路径，以及数据安全回归：**显式 `--key` 唯一性**（指到 NOT NULL UNIQUE 列识别为唯一 → 行级 UPDATE；键值本身变化 → delete+insert；`--where` + 非唯一键 → exit 3 且在 dry-run 与 `--apply` 下均先于任何写入拒绝、dst 零写入）/ **唯一值互换**（两行交换唯一列值：块内转 delete+insert，不升级全量重灌）/ **生成列**（只比较、永不写入；dst 丢列 → 结构同步安全拒绝、dry-run 与 `--apply` 均 exit 2 且数据原样保留）/ **结构 ALTER 失败**（dst 数据装不进 src 列类型：默认原地 ALTER 失败 → 报错 + 数据保留；`--allow-structure-truncate` 显式回退 TRUNCATE 重灌）/ **写路径转义**（dst 全局 `NO_BACKSLASH_ESCAPES` 下反斜杠 / 引号 / 中文值经参数化写入字节级一致，HEX 全表比对）/ **`--snapshot` 并发写入存活** / **只读会话策略探针**（并行扫描期间 performance_schema 断言所有活跃读连接带只读策略、无一漏设）/ **BIGINT 极值切块**（MinInt64~MaxInt64 跨度 → 采样切分）/ **`--sample-limit 0` 不出示例 SQL**）
