...
- sync 的写入语义：缺的行 INSERT、值变的行 UPDATE、多的行 DELETE——**两侧存在兼容且 range-addressable 的共享键时优先采用基于键范围的增量同步**：所谓 range-addressable，是指该键能在两侧以一致的排序/比较语义生成和解释范围边界，并能**稳定定位具体行或键组**（例如普通 INT/UINT 主键、或 collation 一致的字符串键）——它关注的是范围边界是否安全一致，**不是要求键值本身一定唯一**。按键的唯一性分档：**主键或 NOT NULL 唯一索引 → 精确到单行** INSERT/UPDATE/DELETE；**显式指定的非唯一 `--key` → 仅在无 `--where` 时允许，按完整键组执行 DELETE+INSERT 收敛**；**`--where` + 非唯一 `--key` → fail closed（exit 3，零写入）**。dst 上多出来的行（一行杂行，或整段超出 src 键范围）按键逐键删除，行数差异（如 1M 对 1M+1）不改变模式。全量重灌（`TRUNCATE` + 整表回灌）用于**无法按共享键安全做范围寻址**的情况，包括：任一侧无可用键、两侧没有兼容的共享键、键存在但排序语义无法证明安全、ENUM/SET 键（当前实现明确非 range-addressable，即便两侧定义相同）、或行级计划在 pre-pass 与写入之间因数据移动失效且无法安全解释（`--where` 下该情形报参数错）。`TRUNCATE` 是 DDL（隐式提交），dst 用户需要 `DROP` 权限。
...
- **无键表**：整表单块、order-independent 四元组指纹（行数 + ΣH + ⊕H + ΣH²）。语义是**多集合相等**：行序无关、允许重复行，但**无法下钻定位到行**。给表加上**满足 range-addressable 条件的 `--key`** 可升级为按键定位（唯一键精确到行、非唯一键按键组）；该键无法安全做范围寻址时回退无键路径或 fail closed。
