...
- **逐行同步的作用域是 src 的键值范围**：键值落在 src 最小～最大键之外的 dst 行定位不到，首轮删不掉。无 `--where` 时，首轮之后 dst 会多出这些行，第二遍自动升级为全量重灌、对齐；带 `--where` 时范围外的行可能一直留着（复验会如实报 1），需要人工处理。唯一的例外是 src 匹配 `--where` 条件为 0 行：此时直接删光 dst 的匹配行，一遍即可对齐。
...
会。无键表、或 dst 行数比 src 多时走全量重灌，`TRUNCATE` 是 DDL，会把 `AUTO_INCREMENT` 重置为 1。如果 dst 的自增列被别的东西引用（外键、应用缓存），先评估。
...
make build   # 构建
make test    # 单元测试
make e2e     # 端到端：docker 起两个 MySQL 容器跑全部场景（81 项断言）
```
